...
-- BAG_EVENT::EventID
	...
	EVENTBAG_ITR_1 = 34 -- Illusion Temple Renewal 1 (ex803 and later)
	EVENTBAG_ITR_2 = 35 -- Illusion Temple Renewal 2 (ex803 and later)
	EVENTBAG_ITR_3 = 36 -- Illusion Temple Renewal 3 (ex803 and later)
	EVENTBAG_ITR_4 = 37 -- Illusion Temple Renewal 4 (ex803 and later)
	EVENTBAG_ITR_5 = 38 -- Illusion Temple Renewal 5 (ex803 and later)
	EVENTBAG_ITR_6 = 39 -- Illusion Temple Renewal 6 (ex803 and later)
...
-- ====================================================================
	...
	AddItemBag(BAG_EVENT, 2, 0, 'Event_IllusionTemple_Classic_(1-2)_Monsters') -- DropFunction /3/
	AddItemBag(BAG_EVENT, 3, 0, 'Event_IllusionTemple_Classic_(3-4)_Monsters') -- DropFunction /3/
	AddItemBag(BAG_EVENT, 4, 0, 'Event_IllusionTemple_Classic_(5-6)_Monsters') -- DropFunction /3/
	AddItemBag(BAG_EVENT, 5, 0, 'Event_IllusionTemple_Classic_Reward') -- DropFunction /3/
	...
	AddItemBag(BAG_EVENT, 46,0, 'Mix_Cherry_Blossom_White_Reward') -- DropFunction /4/
	AddItemBag(BAG_EVENT, 47,0, 'Mix_Cherry_Blossom_Red_Reward') -- DropFunction /4/
	AddItemBag(BAG_EVENT, 48,0, 'Event_ChaosCastle(2)_Reward') -- DropFunction /3/
	AddItemBag(BAG_EVENT, 49,0, 'Event_ChaosCastle(3)_Reward') -- DropFunction /3/
	AddItemBag(BAG_EVENT, 50,0, 'Event_ChaosCastle(4)_Reward') -- DropFunction /3/
	AddItemBag(BAG_EVENT, 51,0, 'Event_ChaosCastle(5)_Reward') -- DropFunction /3/
	AddItemBag(BAG_EVENT, 52,0, 'Event_ChaosCastle(6)_Reward') -- DropFunction /3/
	AddItemBag(BAG_EVENT, 53,0, 'Event_ChaosCastle(7)_Reward') -- DropFunction /3/
...
function CommonBagItemDrop(aIndex, MapNumber, X, Y)
	...
	local MuunEvoItemID = 0 -- S8E3 only
		...
		if (IsSocketItem(ItemID) == true) then
			if (ItemInfo.Socket > 0 and ItemInfo.Socket <= 5) then -- if slots value is supported (0-5)
				IsSocket = GetRandomValue(ItemInfo.Socket) -- set up to configured value of sockets
			end
		else
		...
		if (IsElementalItem(ItemID) == true) then
			...
			if (ItemID >= 6344 and ItemID <= 6364) then
				if (ItemInfo.Socket > 0 and ItemInfo.Socket <= 5) then
					IsSocket = GetRandomValue(ItemInfo.Socket)
				end
			end
		else
	...
	if (ItemID >= 6344 and ItemID <= 6364 and IsSocket == 0) then
	...
	if (ItemID == MakeItemID(13,211)) then
		...
		IsAncient = 0
		IsSocket = 0
		IsElemental = 0	-- Always 0 if running files as S6E3
	else
		MuunEvoItemID = 0;
	end
	
	CreateItem(aIndex, MapNumber, X, Y, ItemID, ItemLevel, ItemDur, IsSkill, IsLuck, IsOption, aIndex, IsExc, IsAncient, 0, IsSocket, IsElemental, MuunEvoItemID)
	return 1
end
...
function MonsterBagItemDrop(MonsterIndex, MapNumber, MonsterX, MonsterY, PlayerIndex)
	...
	CreateItem(MonsterIndex, MapNumber, MonsterX, MonsterY, ItemID, ItemLevel, ItemDur, IsSkill, IsLuck, IsOption, PlayerIndex, IsExc, IsAncient, 0, IsSocket, IsElemental, MuunEvoItemID)
	return 1
end
...
function EventBagMakeItem()
	...
	return ItemLevel, ItemDur, IsSkill, IsLuck, IsOption, IsExc, IsAncient, IsElemental, IsSocket, MuunEvoItemID
end